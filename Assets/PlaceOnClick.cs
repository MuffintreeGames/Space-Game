    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
            if (canPlace)
            {
                ...
                if (PhotonNetwork.IsConnected) PhotonNetwork.Instantiate(CreatedObject.name, mouseCoords, Quaternion.identity);
                else Instantiate(CreatedObject, mouseCoords, Quaternion.identity);
